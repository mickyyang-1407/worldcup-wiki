  ...
  return (
    <svg viewBox="0 0 120 120" className="w-[130px] h-[130px] md:w-[140px] md:h-[140px] drop-shadow-[0_0_6px_rgba(0,0,0,0.3)]">
      ...
      {gridLevels.map((frac) => {
        ...
        return (
          <polygon
            ...
            points={pts}
            fill="none"
            stroke="var(--radar-grid, rgba(255,255,255,0.06))"
            strokeWidth="0.8"
          />
        );
      })}
      ...
      {Array.from({ length: N }, (_, i) => {
        ...
        return (
          <line key={i} x1={CX} y1={CY} x2={x} y2={y} stroke="var(--radar-grid, rgba(255,255,255,0.06))" strokeWidth="0.8" />
        );
      })}
...
export default function PredictionCard({ prediction: p, highlight }: Props) {
  ...
  // Trend styling
  const trendConfig = {
    up: { icon: '↑', text: '看漲', color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-800/50' },
    down: { icon: '↓', text: '看跌', color: 'text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 border-rose-200 dark:border-rose-800/50' },
    stable: { icon: '→', text: '平穩', color: 'text-gray-600 dark:text-slate-400 bg-gray-50 dark:bg-slate-900/60 border-gray-200 dark:border-slate-800/60' },
  }[p.trend];

  // 1. #1 Hot Contender: Detailed Horizontal Layout
  if (highlight) {
    return (
      <div className="relative rounded-2xl overflow-hidden border border-amber-500/40 bg-gradient-to-br from-amber-50/70 via-indigo-50/40 to-white dark:from-[#1E1B4B]/80 dark:via-[#0F172A]/90 dark:to-[#0F172A]/95 p-6 shadow-[0_0_30px_rgba(245,158,11,0.12)] backdrop-blur-md transition-all duration-300 hover:border-amber-400/60 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col md:flex-row gap-6 items-center">
          ...
          <div className="flex flex-col items-center text-center md:text-left md:items-start gap-4">
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <div>
                <h3 className="text-3xl font-black tracking-tight text-gray-900 dark:text-white flex items-center gap-2">
                  {p.teamNameZh}
                  <span className="text-amber-500 dark:text-amber-400 text-lg font-bold">#1</span>
                </h3>
                <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">{p.teamName} · Group {p.group}</p>
              </div>
            </div>

            {/* Probability summary */}
            <div className="w-full mt-4 bg-gray-100/50 dark:bg-slate-950/40 p-4 rounded-xl border border-gray-200 dark:border-slate-800/50">
              <div className="flex justify-between items-baseline mb-2">
                <span className="text-sm text-gray-600 dark:text-slate-400 font-semibold">奪冠模擬機率</span>
                <span className="text-3xl font-black text-amber-600 dark:text-amber-400 drop-shadow-[0_0_10px_rgba(245,158,11,0.2)]">{pct}%</span>
              </div>
          ...
          {/* Radar Chart */}
          <div className="flex-shrink-0 bg-gray-50/50 dark:bg-slate-900/30 p-2.5 rounded-2xl border border-gray-200 dark:border-slate-850/50 backdrop-blur-sm">
            <RadarChart dims={p.dimensions} highlight={true} />
          </div>
        </div>

        {/* Right Column: 7 Dimensions Detailed Scores */}
        <div className="flex-1 flex flex-col justify-center gap-3 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-slate-800/80 pt-6 lg:pt-0 lg:pl-8">
          <h4 className="text-xs font-extrabold uppercase tracking-widest text-gray-500 dark:text-slate-400 mb-2 flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {RADAR_KEYS.map((k) => {
              ...
              return (
                <div key={k} className="bg-gray-50/50 dark:bg-slate-900/40 p-2.5 rounded-xl border border-gray-150 dark:border-slate-800/50 flex flex-col justify-center gap-1.5 hover:bg-gray-100/70 dark:hover:bg-slate-900/60 transition-colors">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-semibold text-gray-600 dark:text-slate-350 flex items-center gap-1.5">
                      <Icon size={12} className="text-gray-400 dark:text-slate-400" />
                      {config.label}
                    </span>
                    <span className="text-xs font-bold text-gray-900 dark:text-slate-100">{score}/100</span>
                  </div>
                  <div className="h-1.5 bg-gray-250 dark:bg-slate-850 rounded-full overflow-hidden">
                    <div
  ...
  return (
    <div className="relative rounded-2xl border border-gray-250 dark:border-slate-800 bg-white/90 dark:bg-[#0F132E]/70 p-5 shadow-lg backdrop-blur-sm transition-all duration-300 hover:border-gray-300 dark:hover:border-slate-700 hover:shadow-xl flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="font-bold text-gray-900 dark:text-slate-100 text-sm leading-tight">{p.teamNameZh}</h4>
              <p className="text-[10px] text-gray-500 dark:text-slate-400 font-medium tracking-tight leading-tight">{p.teamName}</p>
            </div>
          </div>
        </div>
      ...
      {/* Probability details */}
      <div className="bg-gray-50/55 dark:bg-slate-905/30 p-2.5 rounded-xl border border-gray-150 dark:border-slate-850/50">
        <div className="flex justify-between items-baseline mb-1">
          <span className="text-[11px] text-gray-550 dark:text-slate-400 font-medium">預測機率</span>
      ...
      {/* Radar chart */}
      <div className="flex justify-center py-2 bg-gray-50/50 dark:bg-slate-950/20 rounded-xl border border-gray-150 dark:border-slate-850/30">
        <RadarChart dims={p.dimensions} highlight={false} />
      </div>

      {/* Stats footer */}
      <div className="grid grid-cols-3 gap-1 text-center text-[10px] bg-gray-100/50 dark:bg-slate-950/40 p-2 rounded-lg border border-gray-200 dark:border-slate-850/50">
        <div>
          <p className="text-gray-500 dark:text-slate-400 font-medium mb-0.5">FIFA 排名</p>
          <p className="font-bold text-gray-800 dark:text-slate-200">#{p.dimensions.fifaRanking > 0 ? Math.round((110 - (110 * p.dimensions.fifaRanking / 100))) : '—'}</p>
        </div>
        <div>
          <p className="text-gray-500 dark:text-slate-400 font-medium mb-0.5">小組積分</p>
          <p className="font-bold text-gray-800 dark:text-slate-200">{p.groupPoints} pts</p>
        </div>
        <div>
          <p className="text-gray-500 dark:text-slate-400 font-medium mb-0.5">奪冠賠率</p>
          <p className="font-bold text-gray-800 dark:text-slate-200">{p.odds ? p.odds.toFixed(1) : '—'}</p>
        </div>
      </div>
    </div>
  );
