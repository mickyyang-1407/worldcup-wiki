...
export default function StatsPage() {
  ...
  const upcoming = (matches as any[]).filter((m: any) => m.status === "upcoming");

  const totalGoals = completed.reduce((sum: number, m: any) => sum + m.score.home + m.score.away, 0);
  ...
  const homeWins = completed.filter((m: any) => m.score.home > m.score.away).length;
  const awayWins = completed.filter((m: any) => m.score.home < m.score.away).length;
  const draws = completed.filter((m: any) => m.score.home === m.score.away).length;
        ...
        {topScorers.length > 0 ? (
          <div className="divide-y divide-gray-50">
            {topScorers.map((player, idx) => {
              const team = (teams as any[]).find((t: any) => t.id === player.team);
              return (
