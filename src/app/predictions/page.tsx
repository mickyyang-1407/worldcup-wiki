...
export default async function PredictionsPage() {
  ...
  return (
    <div className="text-gray-800 dark:text-slate-100 flex flex-col">
      ...
      <div className="py-6 flex flex-col gap-10 flex-1 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-8 flex flex-col justify-between bg-gray-50/50 dark:bg-slate-900/40 p-6 rounded-2xl border border-gray-200 dark:border-slate-800/80 backdrop-blur-sm">
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100 mb-1.5 tracking-wide flex items-center gap-2">
              ...
              <div className="mb-6">
                <div className="flex justify-between text-xs font-bold text-gray-500 dark:text-slate-400 mb-2">
                  <span>模型權重分配比例</span>
                  <span className="text-indigo-600 dark:text-indigo-400 font-semibold">Total 100%</span>
                </div>
                <div className="w-full bg-gray-100 dark:bg-slate-950/50 p-1.5 rounded-full border border-gray-200 dark:border-slate-850/80 flex items-center overflow-hidden h-7 shadow-inner">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5">
              {[
                { label: '小組賽成績', weight: '25%', desc: '積分及戰意', color: 'bg-indigo-500', text: 'text-indigo-600 dark:text-indigo-400' },
                { label: '奪冠賠率', weight: '20%', desc: '市場大眾預期', color: 'bg-amber-500', text: 'text-amber-600 dark:text-amber-400' },
                { label: '小組得失球', weight: '15%', desc: '攻防淨勝水準', color: 'bg-blue-500', text: 'text-blue-600 dark:text-blue-400' },
                { label: '近期狀態', weight: '15%', desc: '近期勝負連貫', color: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400' },
                { label: 'FIFA 排名', weight: '10%', desc: '官方戰力趨勢', color: 'bg-yellow-500', text: 'text-yellow-600 dark:text-yellow-400' },
                { label: '媒體聲望', weight: '10%', desc: '聲望關注指標', color: 'bg-purple-500', text: 'text-purple-600 dark:text-purple-400' },
                { label: '攻守指數', weight: '5%',  desc: '進出失球效能', color: 'bg-rose-500', text: 'text-rose-600 dark:text-rose-400' },
              ].map((dim) => (
                <div key={dim.label} className="bg-gray-50 dark:bg-slate-950/40 p-2.5 rounded-xl border border-gray-150 dark:border-slate-850/60 flex flex-col justify-between gap-1.5 hover:border-gray-300 dark:hover:border-slate-700/60 transition-all duration-200">
                  ...
                  <p className="text-[11px] font-bold text-gray-700 dark:text-slate-200 mt-1 leading-tight truncate">{dim.label}</p>
                  <p className="text-[9px] text-gray-400 dark:text-slate-500 leading-tight truncate">{dim.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4">
          <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100 tracking-wide flex items-center gap-2">
